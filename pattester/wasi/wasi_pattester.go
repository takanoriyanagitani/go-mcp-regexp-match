...
// NewWasiPatternTester creates a new PatternTester that uses the WASI WebAssembly module.
// It loads the WASM binary from the given wasmFilePath.
func NewWasiPatternTester(
	ctx context.Context,
	wasmFilePath string,
	memoryLimitPages uint32,
) (rt.PatternTester, func() error, error) {
	...
	return r.ToTester(ctx, compiled, config), cleanup, nil
}
