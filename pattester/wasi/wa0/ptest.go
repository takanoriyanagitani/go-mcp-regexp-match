...
import (
	...
	rt "github.com/takanoriyanagitani/go-mcp-regexp-match"
	"github.com/tetratelabs/wazero"
	wa "github.com/tetratelabs/wazero/api"
)

var (
	ErrInstantiate error = errors.New("internal error: invalid pattern engine")
...
func (c RuntimeConfig) WithPageLimitDefault() RuntimeConfig {
	return c.WithPageLimit(65536) // 4 GiB = 65536 [Page] * 64 [KiB/Page]
}

func (c RuntimeConfig) WithPageLimitSafeDefault() RuntimeConfig {
	return c.WithPageLimit(1024) // 64 MiB = 1024 [Page] * 64 [KiB/Page]
}
...
func (r WasmRuntime) ToTester(ctx context.Context, cmod Compiled, cfg WasmConfig) rt.PatternTester {
	return func(ctx context.Context, input rt.UntrustedInput) rt.PatternTestResult {
		newCfg, e := cfg.WithAutoID()
		if nil != e {
			log.Printf("Failed to generate UUID for WASM instance: %v", e)
			return rt.PatternTestResult{
				IsMatch: false,
				Error:   ErrUuid,
			}
		}
		...
		parsed, e := rt.PatternTestResultDtoFromJson(outputJson)
		if nil != e {
